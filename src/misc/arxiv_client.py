...
from typing import Generator, Tuple, Optional, Iterator
from urllib.parse import urlencode

import feedparser
import requests
from arxiv import Result, Search, _classname, HTTPError, UnexpectedEmptyPageError
...
class Client(object):
    ...
    def _results(
        self, search: Search, offset: int = 0
    ) -> Generator[Tuple[Result, int, int], None, None]:
        ...
        while feed.entries:
            for entry in feed.entries:
                result_index += 1
                try:
                    yield Result._from_feed_entry(entry), result_index, total_results
